...
class FilmeUser(HttpUser):
    ...
    @task
    def ciclo_completo_filme(self):
        if not self.token:
            return

        headers = {"Authorization": f"Bearer {self.token}", "Content-Type": "application/xml", "Accept": "application/xml"}

        xml = f"<?xml version='1.0' encoding='UTF-8'?><filme><titulo>Filme_{fake.uuid4()[:8]}</titulo><duracaoMin>{fake.random_int(60, 180)}</duracaoMin><ano>{fake.random_int(2000, 2024)}</ano></filme>"
